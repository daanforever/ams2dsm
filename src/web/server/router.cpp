...
namespace Web::Server {
  ...
  Router& Router::use(const std::string& path, Middleware middleware) {
    ...
    logger::debug(
        "router.use path_middlewares_ for path {0} size={1}", path, path_middlewares_.count(path)
    );
    return *this;
  }
  ...
  Result Router::apply_middlewares(const Request& req, Response& res) {
    ...
    if (global_middlewares_.size()) {
      ...
      logger::debug("router.apply_middlewares global_middlewares {0}", global_middlewares_.size());
    }

    logger::debug(
        "router.apply_middlewares path_middlewares_ size={1}", req.path, path_middlewares_.size()
    );
  ...
  void Router::chain_to_path_middleware(const std::string& path) {
    ...
    if (chain_middlewares_.size()) {
      logger::debug(
          "router.chain_to_path_middleware chain_middlewares_ size={0}", chain_middlewares_.size()
      );
      for (auto it = chain_middlewares_.rbegin(); it != chain_middlewares_.rend(); ++it) {
        auto& mw = *it;
        logger::debug("router.chain_to_path_middleware use middleware for path {0}", path);
        use(path, mw);
      }
